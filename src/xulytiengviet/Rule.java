...
public class Rule {
    public static boolean checkValidToken(Xau x) {
        ...
        if (checkPhuAmNgat(x) == false) return false;     //kiem tra nguyen am khi phu am cuoi la c, ch, p, t
        return true;
    }
    ...
    private static boolean checkPhuAmNgat(Xau x) {
        if ((x.getPhu_am_cuoi().compareTo("c") == 0) || (x.getPhu_am_cuoi().compareTo("ch") == 0) || (x.getPhu_am_cuoi().compareTo("p") == 0) || (x.getPhu_am_cuoi().compareTo("t") == 0)) {
            for (int i = 0; i < Charcode.nguyenAmDauSacvaNang.length; i++) {
                if (x.getNguyen_am().equals(Charcode.nguyenAmDauSacvaNang[i])) {
                    return true;
                }
            }
            return false;
